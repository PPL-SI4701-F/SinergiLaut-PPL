...
import { getCommunityDashboardStats, getCommunityActivities, getCommunityProfile } from "@/lib/actions/dashboard.actions"
import {
  cancelActivityAction, deleteActivityAction, startActivityAction,
  submitActivityEditRequest, getCommunityActiveEditRequests,
...
const statusConfig: Record<string, { label: string; className: string }> = {
  ...
  ongoing: { label: "Sedang Berlangsung", className: "bg-indigo-100 text-indigo-700" },
...
export default function CommunityDashboardPage() {
  ...
  const [editRequests, setEditRequests] = useState<Record<string, { id: string; status: string; reason: string; admin_note: string | null }>>({})
  const [editRequestActivity, setEditRequestActivity] = useState<CommunityActivity | null>(null)
  ...
  const handleCancelActivity = async (activity: CommunityActivity) => {
    ...
    if (!result.success) {
      ...
      setActivities(prev => prev.map(a => a.id === activity.id ? { ...a, status: "cancelled" } : a))
  ...
  const handleStartActivity = async (activity: CommunityActivity) => {
    ...
    if (!result.success) {
      ...
      setActivities(prev => prev.map(a => a.id === activity.id ? { ...a, status: "ongoing" } : a))
  ...
  const handleSubmitEditRequest = async () => {
    ...
    const result = await submitActivityEditRequest(editRequestActivity.id, trimmed)
    if (result.success) {
      toast.success("Pengajuan edit berhasil dikirim. Menunggu persetujuan admin.")
      setEditRequests(prev => ({
        ...prev,
        [editRequestActivity.id]: { id: result.data?.id ?? "", status: "pending", reason: trimmed, admin_note: null },
      }))
  ...
  const verificationStatus =
    communityProfile?.is_suspended ? "suspended" : communityProfile?.verification_status ?? "pending"

  const verificationCopy: Record<string, { title: string; message: string; badge: string }> = {
    approved: {
      title: "Komunitas Terverifikasi",
      message: "Komunitas Anda sudah disetujui admin dan dapat mengelola kegiatan.",
      badge: "bg-green-100 text-green-700",
    },
    pending: {
      title: "Menunggu Verifikasi Admin",
      message: "Dashboard kegiatan akan aktif setelah data komunitas Anda disetujui admin.",
      badge: "bg-yellow-100 text-yellow-700",
    },
    rejected: {
      title: "Verifikasi Ditolak",
      message: "Perbaiki data komunitas Anda agar admin dapat meninjau ulang pengajuan.",
      badge: "bg-red-100 text-red-700",
    },
    suspended: {
      title: "Komunitas Ditangguhkan",
      message: "Akses pengelolaan kegiatan sedang dibatasi. Hubungi admin untuk informasi lanjutan.",
      badge: "bg-red-100 text-red-700",
    },
  }

  const currentVerification = verificationCopy[verificationStatus] ?? verificationCopy.pending
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-12 md:pt-8">
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm text-primary font-medium">{currentVerification.title}</span>
            </div>
          ...
          {!isLoading && !isCommunityApproved && (
            <Card className="mb-8 border-dashed">
              <CardContent className="p-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <Badge className={currentVerification.badge}>{currentVerification.title}</Badge>
                  <h2 className="text-xl font-semibold mt-3">{communityProfile?.name ?? "Komunitas"}</h2>
                  <p className="text-sm text-muted-foreground mt-1 max-w-2xl">{currentVerification.message}</p>
                </div>
                <Button asChild>
                    ...
                    {filtered.map((a) => (
                      <div key={a.id} className="p-5 border border-border rounded-xl hover:border-primary/30 hover:shadow-sm transition-all bg-background">
                        ...
                        <div className="pt-3 border-t border-border flex gap-2 flex-wrap">
                          ...
                          {a.status === "draft" ? (
                          ...
                          ) : a.status === "published" && editRequests[a.id]?.status === "approved" ? (
                            <Button size="sm" variant="outline" className="flex-1 text-xs h-9 border-primary/20 text-primary hover:bg-primary/5 hover:border-primary/40" asChild>
                              <Link href={`/community/dashboard/activities/${a.id}/edit`}><Edit className="h-3.5 w-3.5 mr-1.5" /> Edit</Link>
                            </Button>
                          ) : a.status === "published" && editRequests[a.id]?.status === "pending" ? (
                            <Button
                              size="sm"
                              variant="outline"
                              disabled
                            ...
                            >
                              <Clock className="h-3.5 w-3.5 mr-1.5" /> Menunggu Persetujuan
                            </Button>
                          ) : a.status === "published" ? (
                            <Dialog open={editRequestActivity?.id === a.id} onOpenChange={(open) => {
                              if (!open) { setEditRequestActivity(null); setEditRequestReason("") }
                            }}>
                              <DialogTrigger asChild>
                                <Button
                                  ...
                                  onClick={() => { setEditRequestActivity(a); setEditRequestReason("") }}
                                >
                                  <Send className="h-3.5 w-3.5 mr-1.5" /> Ajukan Edit
                                </Button>
                              </DialogTrigger>
                              <DialogContent>
                                <DialogHeader>
                                  <DialogTitle>Ajukan Edit Kegiatan</DialogTitle>
                                  <DialogDescription>
                                    Kegiatan &quot;{a.title}&quot; sedang aktif. Jelaskan alasan Anda ingin mengedit kegiatan ini. Admin akan meninjau permintaan sebelum Anda dapat mengedit.
                                  </DialogDescription>
                                </DialogHeader>
                                <Textarea
                                ...
                                />
                                <DialogFooter>
                                  <Button variant="outline" onClick={() => { setEditRequestActivity(null); setEditRequestReason("") }}>Batal</Button>
                                  <Button onClick={handleSubmitEditRequest} disabled={submittingEditRequest}>
                                    {submittingEditRequest ? <Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" /> : <Send className="h-3.5 w-3.5 mr-1.5" />}
                                    Kirim Pengajuan
                                  </Button>
                                </DialogFooter>
                              </DialogContent>
                          ...
                          {a.status === "published" && (
                            <Button
                              size="sm"
                              disabled={!isExecutionOngoing(a.execution_date) || startingId === a.id}
                              title={isExecutionOngoing(a.execution_date)
                          ...
                          {a.status === "pending_review" && (
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  <AlertDialogTitle>Batalkan kegiatan ini?</AlertDialogTitle>
                                  <AlertDialogDescription>
                                    Kegiatan &quot;{a.title}&quot; yang sedang menunggu review akan dibatalkan dan dihapus dari antrean review admin. Tindakan ini tidak dapat diurungkan.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    Kegiatan &quot;{a.title}&quot; yang sudah dibatalkan akan dihapus secara permanen beserta data terkaitnya. Tindakan ini tidak dapat diurungkan.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>Tidak</AlertDialogCancel>
