...
describe('FR-32: Validasi Laporan oleh Admin (/admin/reports)', () => {
    ...
    // ─────────────────────────────────────────────
    it('FR 32 - TC 10 - Harus menolak laporan submitted dan mengubah status menjadi Ditolak', () => {
        cy.contains('tr', /Menunggu Review/i).first().contains('button', /Tolak/i).click();

        cy.contains(/Laporan ditolak/i).should('be.visible');
        cy.contains('tr', /Ditolak/i).should('be.visible');
    });
});
