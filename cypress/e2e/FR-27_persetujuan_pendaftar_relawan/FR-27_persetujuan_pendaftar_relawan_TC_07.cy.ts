describe('FR-27: Persetujuan Pendaftar Relawan - TC-07', () => {
  beforeEach(() => {
    cy.task('resetVolunteerStatus', { activityTitle: 'Rehabilitasi Terumbu Karang Menjangan', status: 'pending' });
  });

  it('Menampilkan tombol Terima dan Tolak untuk relawan pending', () => {
    ...
    // 4 & 5. Cari relawan pending dan verifikasi tombol Terima dan Tolak tersedia
    cy.contains('td', 'Dian Rahmawati')
      .parent('tr')
      .within(() => {
        cy.contains('button', 'Terima').should('be.visible').and('not.be.disabled');
        cy.contains('button', 'Tolak').should('be.visible').and('not.be.disabled');
      });
  });
});
